...
if (TRUE) {
  ...
  rtf.table.out <- function(
    ...
    tb,           # table out
    cell1=NULL,   # can be automatically figured out from column names
    cell2=1, 
    nheader=NULL,  # can be automatically figured out from column names
    ...
    page.disp=TRUE,   #whether diplay page number
    time.disp=TRUE,   #whether display time & date
    font='Courier New',
    Done=TRUE,
    ...
  ){
    ...
    if(is.vector(tb)){
      tb.nm <- names(tb)
      tb <- data.frame(matrix(tb, ncol = length(tb)))
      colnames(tb) <- tb.nm
    }else if (is.matrix(tb)){
      tb.nm <- dimnames(tb)
      tb <- data.frame(tb)
      dimnames(tb) <- tb.nm
    }else if(is.data.frame(tb)){
      #good! no need to do anything
    }else{
      stop('Error: tb must be a data.frame object!')
    }
    ...
    for(k in 1:length(idx)) {
      subTable <- tb[idx[k]:idx.n[k], ] # tb must be a data.frame 
      ...
      ret2 <- rbind(hd, as.matrix(subTable))
      colnames(ret2) <- 1:ncol(ret2)
      rownames(ret2) <- 1:nrow(ret2)
      
      if(is.null(cell1))     cell1 <- nrow(ret2)-nrow(subTable)+1
      if(is.null(nheader)) nheader <- nrow(ret2)-nrow(subTable)
      ...
      if (st > 2) {
        y <- adj.width(x=dat[hd2,],cw=cw, space=1)
        x[hd2,] <- y$x
        cw2[hd2,] <- y$cw
      }
      if (k==1) { # only need do it once
  ...
  #--- this function to be used for single plot output or
